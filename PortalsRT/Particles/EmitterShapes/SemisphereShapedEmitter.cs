...
{
    class SemisphereShapedEmitter : ShapedEmitter
    {
        public Particle[] Emit(ParticleEmitter emitter)
        {
            var particles = new Particle[emitter.ParticlesCount * (emitter.ParticlesCount / 2)];

            float stepAngle = 2 * (float)Math.PI / emitter.ParticlesCount;

            for (int j = 0; j < emitter.ParticlesCount / 2; j++)
            {
